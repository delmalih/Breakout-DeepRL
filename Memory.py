...
class Memory(object):
    def __init__(self, max_memory=constants.MAX_MEMORY):
        ...
        self.states = torch.empty((0, constants.N_CHANNELS, constants.SIZE, constants.SIZE)).to(constants.DEVICE)
        self.next_states = torch.empty((0, constants.N_CHANNELS, constants.SIZE, constants.SIZE)).to(constants.DEVICE)
        self.actions = torch.empty((0,), dtype=torch.long).to(constants.DEVICE)
        self.rewards = torch.Tensor((0,)).to(constants.DEVICE)
        self.dones = torch.Tensor((0,)).to(constants.DEVICE)
